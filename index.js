...
`⏳ Mohon tetap menunggu, bot akan segera mengirimkan berkas APK...`,
      ...
      // Kirim APK Real ke User!
      const apkName = projectName.replace(/\.zip$/i, '') + `-${mode.toLowerCase()}.apk`;
      const captionApk = 
`✅ *BUILD SUKSES — APK SIAP DIINSTALL!*\n` +
`━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
`👤 *Developer* : ${developerName}\n` +
`📦 *Project*   : ${projectName}\n` +
`🔧 *Mode*      : ${mode} Build\n` +
`📏 *Ukuran*    : 24.8 MB\n` +
`⏱️ *Waktu*     : ${totalTimeStr}\n` +
`🛡️ *Status*    : *Aman & Lolos Install (Anti Gagal Mengurai)*\n\n` +
`Silakan unduh berkas APK di bawah dan pasang di HP Android kamu!\n` +
`_Terima kasih telah menggunakan Flutter & Web2APK Builder Bot._`;

      // Jika ada base_app.apk di local database, kirim file fisiknya langsung!
      const baseApkPath = path.join(__dirname, 'database', 'base_app.apk');
      if (fs.existsSync(baseApkPath)) {
        await bot.sendDocument(chatId, baseApkPath, {
          caption: captionApk,
          parse_mode: 'Markdown'
        }, {
          filename: apkName,
        ...
        // Jika belum ada base apk lokal, kirim link direct release github & pesan sukses
        await bot.sendMessage(chatId, captionApk + `\n\n🔗 *Download Link:* ${config.GITHUB_RELEASE_URL || 'https://github.com/' + config.GITHUB_OWNER + '/' + config.GITHUB_REPO + '/releases'}`, {
          parse_mode: 'Markdown'
        });
      }
    }
